...
create table if not exists `local_delay_message`
(
...
create index idx_business_id
    on local_delay_message (business_id, status);

create index idx_mq_platform_msg
    on local_delay_message (platform, platform_msg_id);

create index idx_time_status
    on local_delay_message (time, status);